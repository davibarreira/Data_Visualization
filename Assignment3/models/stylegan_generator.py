...
class SynthesisModule(nn.Module):
    ...
    def gather_style_mappings(self):
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            block_idx = res_log2 - self.init_res_log2
            if block_idx == 0:
                style_mappings.append({'layer':0,'map':self.layer0.get_style_map()})
            else:
                style_mappings.append({'layer':2*block_idx,'map':self.__getattr__(f'layer{2 * block_idx}').get_style_map()})
            #
            style_mappings.append({'layer':2*block_idx+1,'map':self.__getattr__(f'layer{2 * block_idx + 1}').get_style_map()})
            #
        #
    ...
    def forward(self, wp, lod=None, randomize_noise=False):
        ...
        for res_log2 in range(self.init_res_log2, self.final_res_log2 + 1):
            ...
            if lod < current_lod + 1:
                ...
                if block_idx == 0:
                    if self.const_input:
                        x, style, raw_conv = self.layer0(None, wp[:, 0], randomize_noise)
                    else:
                        x = wp[:, 0].view(-1, self.w_space_dim, 1, 1)
                        x, style, raw_conv = self.layer0(x, wp[:, 0], randomize_noise)
                else:
                    x, style, raw_conv = self.__getattr__(f'layer{2 * block_idx}')(
                        x, wp[:, 2 * block_idx])
                    layer_id = block_idx
                #
                if (2*block_idx) >= 2 and (2*block_idx) <= 4:
                    results[f'act{2 * block_idx:01d}'] = raw_conv
                x, style, raw_conv = self.__getattr__(f'layer{2 * block_idx + 1}')(
                    x, wp[:, 2 * block_idx + 1])
                if (2*block_idx+1) >= 2 and (2*block_idx+1) <= 4:
                    results[f'act{2 * block_idx + 1:01d}'] = raw_conv
...
class ConvBlock(nn.Module):
    ...
    def forward(self, x, w, randomize_noise=False):
        ...
        normalized_x = self.normalize(activation)
        x, style = self.style(normalized_x, w)
        return x, style, activation
